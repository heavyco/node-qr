...
Encoder.prototype.default_options = {
    ...
    type : 'PNG' // Default to PNG output
};

Encoder.prototype.types = ['PNG','EPS','SVG','ANSI','ANSI256','ASCII','ASCIIi','UTF8','ANSIUTF8'];
...
Encoder.prototype.encode = function(value, path, options)
{
    ...
    try {
        ...
        // Throw an error if the type is not accepted
        if(self.types.indexOf(cmd_options.type) === -1){
            throw new Error('type must be one of', self.types);
        }
